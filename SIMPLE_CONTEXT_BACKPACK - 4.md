...
⚠️ مبدأ التنفيذ والموافقة (مهم جداً):
  الأفكار_الجديدة: "اعرض الفكرة أولاً واحصل على الموافقة قبل التنفيذ"
...
npx prisma migrate dev           -- سيعلق

-- ✅ الطريقة الصحيحة الوحيدة:
await prisma.$executeRawUnsafe(`CREATE TABLE...`);
```

### 🚨 قيود Supabase الحرجة (لا تجاهل)

#### المشاكل الأكثر تكراراً وحلولها:
...
### المبادئ الأساسية
```yaml
الاستقلالية_الكاملة: "كل جمعية لها قاعدة بيانات Supabase منفصلة"
المرونة_التقنية: "Supabase حصرياً مع إعدادات مرنة لكل جمعية"
...
## �️ بيئة التطوير وأدوات MCP

## �️ الخريطة التقنية المبسطة

### ⚙️ إعدادات VS Code الحرجة
...
```yaml
البساطة: "استخدم الأدوات حسب الحاجة فقط"
الأمان: "GitHub token محفوظ آمن في User Settings"
المرونة: "Filesystem يتكيف مع أي مشروع تلقائياً"
الاستقرار: "لا إعدادات متكررة - ضبط مرة واحدة"
الكفاءة: "تنفيذ فوري بدون انتظار تأكيدات"
```

### 📋 بروتوكول MCP في كل جلسة

```bash
✅ الأدوات جاهزة تلقائياً (لا حاجة للتكوين)
✅ GitHub MCP: للعمل مع repositories
✅ PostgreSQL MCP: عند التعامل مع البيانات
✅ Filesystem MCP: للعمليات المعقدة على الملفات
...
### نظام إدارة الحلقات للمعلمة (أساسي)
```typescript
interface TeacherCourseManagement {
  workflow: "Dashboard → اختيار الحلقة → الصفحات المفلترة",
  mainPage: "/teacher",
  courseSelection: "واجبة قبل أي إجراء",
  filtering: "جميع البيانات مفلترة حسب courseId",
  apis: "دعم معامل courseId في جميع APIs"
}
```
...
interface SimpleGrading {
  prerequisite: "نظام اختيار الحلقة يجب أن يعمل أولاً",
  version1: "درجة واحدة بسيطة لكل طالبة (مع courseId)",  // البداية
  version2: "درجات متعددة حسب المواد (مفلترة حسب الحلقة)",      // لاحقاً
...
### الهدف المحدد
**إنشاء مشروج Next.js 15 يعمل مع 3 صفحات بسيطة**

### الصفحة الرئيسية الحالية
**الموقع:** `src/app/page.tsx`
**المحتوى:** "منصة شموخ التعليمية v3" مع 3 أزرار (تسجيل الدخول، لوحة التحكم، الملف الشخصي)
**التصميم:** بطاقة بيضاء وسط خلفية متدرجة من الأزرق
...
src/app/teacher/page.tsx               // ✅ صفحة اختيار الحلقة للمعلمة
src/app/enrolled-students/page.tsx     // ✅ واضح (مع دعم courseId)
src/app/attendance/page.tsx            // ✅ واضح (مع نظام الحفظ المجمع)
