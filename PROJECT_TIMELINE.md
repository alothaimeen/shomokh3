...
**الجلسة الحالية:** 16.1 مكتملة ✅ + تحسينات  
**Build Status:** ✅ ناجح (61 routes)  
**التقدم:** 16.1/35 جلسة (~46%)  
**الجلسة القادمة:** 17 - التقارير الأساسية
...
- Section 1 (الدرجات): 5 حقول (4 درجات يومية + سلوك) - خلفية بيضاء
...
**1. إصلاح خطأ في فهم الدرجات اليومية:**
- ❌ الخطأ: قسمت الدرجات إلى 4 حقول (حفظ.قرآن، حفظ.تجويد، مراجعة.قرآن، مراجعة.تجويد)
- ✅ الصحيح: خانتان فقط كما في قاعدة البيانات
  * حفظ وتجويد (0-5) - حقل واحد `memorization`
  * مراجعة وتجويد (0-5) - حقل واحد `review`
  * درجة السلوك (0-1) - حقل واحد `behaviorScore`
- النتيجة: 11 درجة كحد أقصى يومياً (5+5+1)
- الملف: `src/components/assessment/DailyGradesTab.tsx` (تم الإصلاح)

**2. إصلاح خطأ toFixed:**
- المشكلة: `totals.grandTotal.toFixed is not a function`
- السبب: قيم select تُرجع strings
- الحل: إضافة `Number()` في دالة `calculateTotals`

**3. إضافة حقل الحضور:**
- ✅ إضافة قسم الحضور أعلى البطاقة
- خلفية زرقاء فاتحة `bg-indigo-50`
...
- الربط: جلب من `/api/attendance/course-attendance`
- الحفظ: استخدام `/api/attendance/mark` لكل طالبة
- الملف: `src/components/assessment/DailyGradesTab.tsx` (محدث)

**البطاقة النهائية تحتوي:**
1. 📋 الحضور (قائمة منسدلة)
2. 📚 الدرجات (حفظ 0-5 + مراجعة 0-5 + سلوك 0-1)
3. 🎯 المهام (سماع، تكرار، سرد - read-only)
4. 🏆 النقاط (4 معايير سلوكية)
5. ملخص: درجات | مهام | نقاط

**المجموع اليومي:** 46 نقطة كحد أقصى (11 درجات + 15 نقطة مهام + 20 نقطة سلوك)
